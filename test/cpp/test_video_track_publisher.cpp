...
TEST(VideoTrackPublisherTest, RepublishesOnSizeChangeAndUnpublishesOnDestruction)
{
  ...
  publisher->captureFrame(makeFrame(2, 2), 1000);
  publisher->captureFrame(makeFrame(2, 2), 2000);
  publisher->captureFrame(makeFrame(4, 4), 3000);
  publisher.reset();
  publisher.reset();
...
TEST(VideoTrackPublisherTest, PublishFailureOnFirstFrameCanRetryAndStillDestroyCleanly)
{
  ...
  EXPECT_THROW(publisher->captureFrame(makeFrame(2, 2), 1000), std::runtime_error);
  publisher->captureFrame(makeFrame(2, 2), 2000);
  publisher.reset();
...
TEST(VideoTrackPublisherTest, RepublishFailureLeavesPublisherReadyForRetry)
{
  ...
  publisher->captureFrame(makeFrame(2, 2), 1000);
  EXPECT_THROW(publisher->captureFrame(makeFrame(4, 4), 2000), std::runtime_error);
  publisher->captureFrame(makeFrame(4, 4), 3000);
  publisher.reset();
...
TEST(VideoTrackPublisherTest, BlockingPublishStillUnpublishesOnceOnDestruction)
{
  ...
  auto write_future =
    std::async(std::launch::async, [&publisher]() { publisher->captureFrame(makeFrame(2, 2), 1000); });
