...
static GstStateChangeReturn roscompressedimagesrc_change_state(GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY: {
      ...
      GST_INFO_OBJECT(
        self,
        "event=source_started topic=%s kind=compressed_image reliable=%s resource=subscription",
        self->ros_topic,
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      if (self->sub || self->node_runner) {
        GST_INFO_OBJECT(
          self,
          "event=source_stopped topic=%s kind=compressed_image reliable=%s resource=subscription reason=%s",
          self->ros_topic,
          self->ros_reliable ? "true" : "false",
...
static GstFlowReturn roscompressedimagesrc_create(GstPushSrc * push_src, GstBuffer ** buf)
{
  ...
  *buf = gst_buffer_new_allocate(NULL, length, NULL);
  if (*buf == NULL) {
    GST_ERROR_OBJECT(
      self,
      "event=buffer_allocate_failed topic=%s kind=compressed_image reliable=%s resource=buffer "
